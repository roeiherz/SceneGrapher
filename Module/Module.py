...
class Module(object):
    ...
    def nn_predicate_weights(self, in_size, out_size):
        ...
        with tf.variable_scope("nn_predicate_weights"):
            ...
            # Define batch-norm after the addition
            self.nn_predicate_beta_6 = tf.get_variable(name="beta6", shape=(out_size,),
                                                       initializer=tf.zeros_initializer())
            self.nn_predicate_gamma_6 = tf.get_variable(name="gamma6", shape=(out_size,),
    ...
    def nn_predicate(self, features, in_belief_predicate, out_shape, scope_name="nn_predicate"):
        ...
        with tf.variable_scope(scope_name):
            ...
            # Batch Norm
            z = self.batch_norm(x=y, beta=self.nn_predicate_beta_6, gamma=self.nn_predicate_gamma_6)
            out = tf.nn.softmax(z, name="out")

            # reshape to fit the required output dims
            z = tf.reshape(z, out_shape)
    ...
    def rnn_stage(self, in_visual_features_predicate, in_visual_features_object, in_belief_predicate, in_belief_object,
        ...
        with tf.variable_scope(scope_name):
            with tf.variable_scope("feature_collector"):
                ...
                # predicate_beliefs = tf.concat((in_belief_predicate, expand_belief_subject, expand_belief_object),
                #                               axis=2, name="predicate_beliefs")

                predicate_beliefs = tf.concat(
                ...
                predicate_features = tf.concat((in_visual_features_predicate, expand_global_sub_belief,
                                                expand_global_obj_belief), axis=2, name="predicate_features")
            ...
            out_belief_predicate, last_layer_predicate = self.nn_predicate(predicate_beliefs, in_belief_predicate,
    ...
    def batch_norm(self, x, beta, gamma, scope='batch-normalization'):
        """
        Reference: http://arxiv.org/abs/1502.03167

        "Batch Normalization: Accelerating Deep Network Training by Reducing Internal Covariate Shift"
        :param x: Tensor, 4D BHWD input maps
        :param beta: TF Constant Variable, beta parameter
        :param gamma: TF Constant Variable, gamma parameter
        ...
        with tf.variable_scope(scope):
            ...
            mean, var = tf.cond(tf.cast(self.is_train, tf.bool),
                                mean_var_with_update,
                                lambda: (ema.average(batch_mean), ema.average(batch_var)))

            normed = tf.nn.batch_normalization(x, mean, var, beta, gamma, variance_epsilon=1e-3)
        return normed
