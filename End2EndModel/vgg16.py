...
class vgg16:
    def __init__(self, imgs, weights=None, sess=None, scope="vgg16", reuse=None):
        self.imgs = imgs
        self.reuse = reuse
        with tf.variable_scope(scope) as scope:
            self.convlayers()
            self.fc_layers()
            self.features = self.fc3l
    ...
    def convlayers(self):
        ...
        # zero-mean input
        with tf.variable_scope('preprocess', reuse=self.reuse) as scope:
            mean = tf.constant([123.68, 116.779, 103.939], dtype=tf.float32, shape=[1, 1, 1, 5], name='img_mean')
            images = self.imgs-mean

        # conv1_1
        with tf.variable_scope('conv1_1', reuse=self.reuse) as scope:
            kernel = tf.get_variable(initializer=tf.truncated_normal([3, 3, 5, 64], dtype=tf.float32,
        ...
        # conv1_2
        with tf.variable_scope('conv1_2', reuse=self.reuse) as scope:
        ...
        # conv2_1
        with tf.variable_scope('conv2_1', reuse=self.reuse) as scope:
        ...
        # conv2_2
        with tf.variable_scope('conv2_2', reuse=self.reuse) as scope:
        ...
        # conv3_1
        with tf.variable_scope('conv3_1', reuse=self.reuse) as scope:
        ...
        # conv3_2
        with tf.variable_scope('conv3_2', reuse=self.reuse) as scope:
        ...
        # conv3_3
        with tf.variable_scope('conv3_3', reuse=self.reuse) as scope:
            kernel = tf.get_variable(initializer=tf.truncated_normal([3, 3, 256, 256], dtype=tf.float32,
                                                     stddev=1e-1), name='weights')
        ...
        # conv4_1
        with tf.variable_scope('conv4_1', reuse=self.reuse) as scope:
        ...
        # conv4_2
        with tf.variable_scope('conv4_2', reuse=self.reuse) as scope:
        ...
        # conv4_3
        with tf.variable_scope('conv4_3', reuse=self.reuse) as scope:
            kernel = tf.get_variable(initializer=tf.truncated_normal([3, 3, 512, 512], dtype=tf.float32,
                                                     stddev=1e-1), name='weights')
        ...
        # conv5_1
        with tf.variable_scope('conv5_1', reuse=self.reuse) as scope:
            kernel = tf.get_variable(initializer=tf.truncated_normal([3, 3, 512, 512], dtype=tf.float32,
                                                     stddev=1e-1), name='weights')
        ...
        # conv5_2
        with tf.variable_scope('conv5_2', reuse=self.reuse) as scope:
            kernel = tf.get_variable(initializer=tf.truncated_normal([3, 3, 512, 512], dtype=tf.float32,
                                                     stddev=1e-1), name='weights')
        ...
        # conv5_3
        with tf.variable_scope('conv5_3', reuse=self.reuse) as scope:
            kernel = tf.get_variable(initializer=tf.truncated_normal([3, 3, 512, 512], dtype=tf.float32,
                                                     stddev=1e-1), name='weights')
    ...
    def fc_layers(self):
        ...
        with tf.variable_scope('fc1', reuse=self.reuse) as scope:
            ...
            fc1w = tf.get_variable(initializer=tf.truncated_normal([shape, 50],
                                                         dtype=tf.float32,
                                                         stddev=1e-1), name='weights')
            fc1b = tf.get_variable(initializer=tf.constant(1.0, shape=[50], dtype=tf.float32),
                                 trainable=True, name='biases')
